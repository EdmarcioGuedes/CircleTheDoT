...
function colorBlackCircles() {
	...
	if (youWin || youLose) {
		...
		document.getElementById("divWinLose").style.display = "none";
...
function moveRed() {
	...
	for (let i = 0; i < 6; i++) {
		...
		if (arrayCircles[possibleNewReds[i]][2] == "azure") {
			...
			timesRedMoved++;
			document.getElementById("pYourScore").innerHTML = timesRedMoved;
...
function checkWinOrLose() {
	...
	if (youLose) {
		x.style.display = "block";
		document.getElementById("divWinLose").innerHTML = "You Lose!<br> Try again!";
		cleanNewRed();
	}
}


function cleanNewRed() {

	for (let i = 0; i < arrayCircles.length; i++) {

		if (arrayCircles[i][2] == "red") {
			arrayCircles[i][2] = "azure";
		}
	}
}
